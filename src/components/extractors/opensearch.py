...
class OpensearchExtractor(BaseExtractor):
    """
    Extracts documents from OpenSearch as a data source.
    ...
    def __init__(self, connection: OpenSearch, config: Dict[str, Any]):
        ...
        # Fail-fast validation for incremental mode
    ...
    def _bulk_extract(self) -> Iterator[Dict[str, Any]]:
        logger.info(
            "Starting BULK extraction from OpenSearch index '%s'",
    ...
    def _incremental_extract(self) -> Iterator[Dict[str, Any]]:
        logger.info(
            "Starting INCREMENTAL extraction from OpenSearch index '%s'",
            self.config.index_name,
        )
